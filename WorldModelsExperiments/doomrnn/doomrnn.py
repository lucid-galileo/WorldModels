...
TEMPERATURE = 1.15 # train with this temperature
...
class ConvVAE(object):
  ...
  def _build_graph(self):
    ...
    with self.g.as_default():
      ...
      self.assign_ops = {}
      for var in t_vars:
          if True: # var.name.startswith('conv_vae'): # idk why variable names aren't including "conv_vae"
...
class Model():
  ...
  def build_model(self, hps):
    ...
    self.assign_ops = {}
    for var in t_vars:
        if True:  #var.name.startswith('mdn_rnn'): # idk why mdn_rnn isn't showing up in name
